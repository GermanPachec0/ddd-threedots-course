...
func TestNewEventBus(t *testing.T) {
	...
	someEvents, err := pubSub.Subscribe(context.Background(), "DocumentPrinted")
	require.NoError(t, err)

	anotherEvents, err := pubSub.Subscribe(context.Background(), "DocumentDownloaded")
	require.NoError(t, err)
	...
	case <-time.After(100 * time.Millisecond):
		require.Fail(t, "no DocumentPrinted event received on DocumentPrinted topic")
	}
	...
	case <-time.After(100 * time.Millisecond):
		require.Fail(t, "no DocumentDownloaded event received on DocumentDownloaded topic")
	}
}
